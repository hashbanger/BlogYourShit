...
{}
If you did not make this request, ignore the email, no changes will be made.'''.format(url_for('reset_token',token=  token, _external = True))
    mail.send(msg)
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('An email has been set with instructions to reset a password!')
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Your password has been updated! Try logging in.', category = 'success')
        return redirect(url_for('login'))
